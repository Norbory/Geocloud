...
function Map() {
  ...
  const [marcador, setMarcador] = useState<{
    altura: ReactNode;
    status: ReactNode; position: { lat: number; lng: number }; content: string 
} | null>(null);
        ...
        >
          <NavLink to="/device" className="flex">
            <div>
              ...
              {(marcador.altura && +marcador.altura < 15) && (+marcador.altura > 10) ? (
                <p className="ml-1 text-yellow-300">Alerta Amarilla</p>
              ) :(marcador.altura && +marcador.altura < 10) && (+marcador.altura > 5) ? (
                <p className="ml-1 text-amber-600">Alerta Naranja</p>
              ) : marcador.altura && +marcador.altura < 5 ? (
